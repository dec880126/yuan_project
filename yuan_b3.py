class SimulateSaving:
    ...
    def simulate(self, portionSavings=0.1) -> int:
        ...
        while currentSavings < fisrtCost:
            ...
            currentSavings += (
                monthlySalary * portionSavings
            )  # save money by parameter: portionSavings
            monthToSave += 1
        return monthToSave
...
class SearchSavingsRate(SimulateSaving):
    def binarySearch(self, monthsToSave=36):
        ...
        while (
            month != monthsToSave
        ):  # keep trying until the number of month has been tried equal to parameter: monthsToSave
            month = super().simulate(
                portionSavings
            )  # super(): use the function that defined in mother's class
...
class SearchSavingsRateWRaise(SearchSavingsRate):
    def simulate(self, portionSavings=0.1) -> int:
        ...
        while currentSavings < fisrtCost:
            print(
                f"今は第{monthToSave}月                           預金は{currentSavings:.1f}円"
            )
